  ...
  if ((errorCode & WRONG_BOOT_REASON) == WRONG_BOOT_REASON){
    if (volt >= 3.6){
      ...
      if (GSMsetup()){
	    ...
	    /*
	  	int HTTPtimeOut = LongLine.length() + 1000;
        _D(DebugPrintln("HTTP Line Length: " + String(HTTPtimeOut), DEBUG_LEVEL_3));
        _D(DebugPrintln("HTTP TimeOut    : " + String(LongLine.length()), DEBUG_LEVEL_3));
        int HTTPrc = GSMdoPost("https://sx8y7j2yhsg2vejk.myfritz.net:1083/push_data.php",
