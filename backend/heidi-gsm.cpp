...
bool openGSM(){
  ...
  if (!GSMenabled){
    ...
    gpio_pullup_dis(GSM_RXD);
    gpio_pulldown_en(GSM_RXD);
...
bool hGSMsetup()
{
  ...
  for(int z=0; z<2; z++){
  	if (z>0){
	    resp = hGSMsendCommand("AT+CPOWD=0");
	    delay(500);
	  }
    // get info
    _D(DebugPrintln(modemGSM.getModemInfo(), DEBUG_LEVEL_2));
    // init modem
    if (!modemGSM.restart())
    {
    ...
    _D(DebugPrintln("Modem restart OK", DEBUG_LEVEL_2));
    //unlock SIM
    ...
    */
    ...
    if(!hGSMsetupGPRS("web.vodafone.de", "", ""))
    {
      _D(DebugPrintln("GPRS Connection\nFailed", DEBUG_LEVEL_1));
      delay(1000);
      continue;
    }
    _DD(DebugPrintln("GPRS Connect OK", DEBUG_LEVEL_3));


    //configure DNS to vodafone DNS
    response = hGSMsendCommand("AT+CDNSCFG=\"139.007.030.125\",\"139.007.030.126\"");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1) {
      _D(DebugPrintln("SIM800L : doPost() - Unable to define vodafone DNS", DEBUG_LEVEL_1));
    }else {
      response = hGSMsendCommand("AT+CSQ");
	  _DD(DebugPrintln("SIM800L : response to \"AT+CSQ\": " + response, DEBUG_LEVEL_3));
      return true;
    }
  } //for z
  return false;
...
bool hGSMsetupGPRS(const String apn, const String user, const String pwd)
{
    ...
    _DD(DebugPrintln("SIM800L : shut IP sessions.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+CIPSHUT"); // Reset the IP session if any
    ...
    _DD(DebugPrintln("SIM800L : set the connection type to GPRS.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+SAPBR=3,1,\"Contype\",\"GPRS\"");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : set APN.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+SAPBR=3,1,\"APN\",\"" + apn + "\"");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : set USER.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+SAPBR=3,1,\"USER\",\"" + user + "\"");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : set PWD.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+SAPBR=3,1,\"PWD\",\"" + pwd + "\"");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : open the defined GPRS bearer context.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+SAPBR=1,1");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : query the GPRS bearer context status.", DEBUG_LEVEL_3));
    ...
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
  	  _DD(DebugPrintln("SIM800L : Cannot set to multi-IP.", DEBUG_LEVEL_1));
  	  delay(200);
  	  //return false;
    }
    */
    // Put in "quick send" mode (thus no extra "Send OK")
    _DD(DebugPrintln("SIM800L : enable quick send mode.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+CIPQSEND=1");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : set to get data manually.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+CIPRXGET=1");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : start Task and Set APN, USER NAME, PASSWORD.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+CSTT=\"" + apn + "\",\"" + user + "\",\"" + pwd + "\"");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : bring Up Wireless Connection with GPRS.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+CIICR");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
    ...
    _DD(DebugPrintln("SIM800L : get Local IP Address.", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+CIFSR;E0");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1){
...
bool hGSMshutDown()
{
  String resp = hGSMsendCommand("AT+CIPSHUT"); // Reset the IP session if any
...
*/
  resp = hGSMsendCommand("AT+CREG=0");
  if(resp.indexOf("OK") == -1){
  	_D(DebugPrintln("SIM800L : Cannot unregister SIM.", DEBUG_LEVEL_1));
  	delay(200);
  	return false;
  }
  resp = hGSMsendCommand("AT+CPOWD=1");
  delay(100);
  SerialGSM.end();
  return true;
}
...
int hGSMdoPost(String url, String contentType, String payload, unsigned int clientWriteTimeoutMs, unsigned int serverReadTimeoutMs) {
  ...
  response = hGSMsendCommand("AT+CCLK?");
  if(response.indexOf("OK") != -1){
	_DD(DebugPrint("GSM time : " + response.substring(7, response.indexOf((char)0x0C)) , DEBUG_LEVEL_3));
  }
  // Initiate HTTP/S session with the module
  int initRC = hGSMinitiateHTTP(url);
  if(initRC > 0) { return initRC; }
  // Define the content type
  _DD(DebugPrint("AT+HTTPPARA=\"CONTENT\",\"" + contentType + "\"", DEBUG_LEVEL_3));
  response = hGSMsendCommand("AT+HTTPPARA=\"CONTENT\",\"" + contentType + "\"");
  _DD(DebugPrintln(response, DEBUG_LEVEL_3));
  if(response.indexOf("OK") == -1) {
  ...
  _DD(DebugPrint("AT+HTTPDATA=" + String(l) + "," + String(clientWriteTimeoutMs), DEBUG_LEVEL_3));
  response = hGSMsendCommand("AT+HTTPDATA=" + String(payload.length()) + "," + String(clientWriteTimeoutMs));
  _DD(DebugPrintln(response, DEBUG_LEVEL_3));
  ...
  _DD(DebugPrintln("SIM800L : doPost() - send " + String(l) + " bytes payload", DEBUG_LEVEL_3));
  for (int i=0; i<l; i++){
  ...
  _DD(DebugPrint("AT+HTTPACTION=1", DEBUG_LEVEL_3));
  response = hGSMsendCommand("AT+HTTPACTION=1", 30000);
  _DD(DebugPrint(response, DEBUG_LEVEL_3));
  if(response.indexOf("OK") == -1) {
  ...
  while(millis() - t < serverReadTimeoutMs){
    delay(1);
    response += SerialGSM.readString();
    if (response.length() > 0) { _DD(DebugPrintln(response, DEBUG_LEVEL_3)); }
  ...
  if(dataSize >= 2) { //need to find "OK" inside, so at least 2 Bytes
    ...
    response = hGSMsendCommand("AT+HTTPREAD");
    _DD(DebugPrintln("SIM800L : doPost() - response to \"AT+HTTPREAD\": " + response, DEBUG_LEVEL_3));
    ...
    httpResponseData = response.substring(startPayload, startPayload + dataSize);
...
int hGSMdoGet(const char* url, unsigned int serverReadTimeoutMs) {
  ...
  String response = hGSMsendCommand("AT+HTTPACTION=0");
...
int hGSMinitiateHTTP(String url) {
  ...
  _DD(DebugPrint("AT+HTTPINIT", DEBUG_LEVEL_3));
  response = hGSMsendCommand("AT+HTTPINIT");
  _DD(DebugPrintln(response, DEBUG_LEVEL_3));
  ...
  // Use the GPRS bearer
  _DD(DebugPrint("AT+HTTPPARA=\"CID\",1", DEBUG_LEVEL_3));
  response = hGSMsendCommand("AT+HTTPPARA=\"CID\",1");
  _DD(DebugPrintln(response, DEBUG_LEVEL_3));
  if(response.indexOf("OK") == -1) {
  ...
  _DD(DebugPrint("AT+HTTPPARA=\"URL\",\"" + url + "\"", DEBUG_LEVEL_3));
  response = hGSMsendCommand("AT+HTTPPARA=\"URL\",\"" + url + "\"");
  _DD(DebugPrintln(response, DEBUG_LEVEL_3));
  if(response.indexOf("OK") == -1) {
  ...
  if(url.indexOf("https://") == 0) {
    _DD(DebugPrint("AT+HTTPSSL=1", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+HTTPSSL=1");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1) {
    ...
    _DD(DebugPrint("AT+HTTPSSL=0", DEBUG_LEVEL_3));
    response = hGSMsendCommand("AT+HTTPSSL=0");
    _DD(DebugPrintln(response, DEBUG_LEVEL_3));
    if(response.indexOf("OK") == -1) {
...
int hGSMterminateHTTP() {
  ...
  String response = hGSMsendCommand("AT+HTTPTERM");
  if(response.indexOf("OK") == -1)  {
...
String hGSMsendCommand(const String command){
  return hGSMsendCommand(command, 5000);
}
...
void hGSMCheckSignalStrength(){
  ...
  if (getError(WRONG_BOOT_REASON)){
    ...
    if (hGSMsetup()){
	  ...
	  response = hGSMsendCommand("AT+CGSN");
    ...
    if (HTTPrc == 200){
      _D(DebugPrintln("HTTP response: " + hGSMGetLastResponse(), DEBUG_LEVEL_1));
	  	String fenceData = GetCSVvalue(hGSMGetLastResponse(), 2);
      _D(DebugPrintln("fence Data: " + fenceData, DEBUG_LEVEL_1));
      if (fenceData.length() > 4){
        clearFence();
        if(!newFenceB64(fenceData)){
          _D(DebugPrintln("set fence failed", DEBUG_LEVEL_1));
        }
      }
    }
    */
