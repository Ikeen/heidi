...
bool hGSMsetup()
{
  ...
  for(int z=0; z<2; z++){
    ...
    // get info
    _D(DebugPrintln(modemGSM.getModemInfo(), DEBUG_LEVEL_3));
    // init modem
    if (!modemGSM.restart())
    {
    ...
    */
    ...
    if(response.indexOf("OK") == -1) {
	  ...
	  _D(DebugPrintln("SIM800L : response to \"AT+CSQ\": " + response, DEBUG_LEVEL_3));
      return true;
    }
  } //for z
  return false;
...
int hGSMdoPost(String url, String contentType, String payload, unsigned int clientWriteTimeoutMs, unsigned int serverReadTimeoutMs) {
  ...
  if(response.indexOf("OK") != -1){
	_D(DebugPrint("GSM time : " + response.substring(7, response.indexOf((char)0x0C)) , DEBUG_LEVEL_3));
  }
  ...
  if(dataSize >= 2) { //need to find "OK" inside, so at least 2 Bytes
    ...
    _D(DebugPrintln("SIM800L : doPost() - response to \"AT+HTTPREAD\": " + response, DEBUG_LEVEL_3));
